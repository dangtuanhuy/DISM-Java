...
public class tbTransaction {
    ...
    public ArrayList<tbTransaction> getList() {
        ...
        try {
            ...
            pst = cn.prepareCall("SELECT a.ID, b.Cus_Name, c.ATM_Name, d.ATMCardID, a.TS_Manipulation, a.TS_Money, a.TS_Date, a.TS_Status, a.TS_Customer_Sub FROM (((GiaoDich a JOIN KhachHang b ON a.TS_CustomerID = b.ID) JOIN ATM c ON a.ATMID = c.ID) JOIN ATMCard d ON a.ATMCardID = d.ID)");
    ...
    public ArrayList<tbTransaction> fillter(int action, int date) {
        ...
        ArrayList<tbTransaction> listCus = new ArrayList();
        try {
            sql = "SELECT a.ID, b.Cus_Name, c.ATM_Name, d.ATMCardID, a.TS_Manipulation, a.TS_Money, a.TS_Date, a.TS_Status, a.TS_Customer_Sub FROM (((GiaoDich a JOIN KhachHang b ON a.TS_CustomerID = b.ID) JOIN ATM c ON a.ATMID = c.ID) JOIN ATMCard d ON a.ATMCardID = d.ID)" + query + act + and + d;
            cn = MyConnection.getConnect(server.getServerName(), server.getDatabaseName(), server.getUserName(), server.getPassword());
    ...
    public ArrayList<tbTransaction> getTransactionByKHID(int khid) {
        ...
        try {
            cn = MyConnection.getConnect(server.getServerName(), server.getDatabaseName(), server.getUserName(), server.getPassword());
            pst = cn.prepareCall("SELECT a.ID, b.Cus_Name, c.ATM_Name, d.ATMCardID, a.TS_Manipulation, a.TS_Money, a.TS_Date, a.TS_Status, a.TS_Customer_Sub FROM (((GiaoDich a JOIN KhachHang b ON a.TS_CustomerID = b.ID) JOIN ATM c ON a.ATMID = c.ID) JOIN ATMCard d ON a.ATMCardID = d.ID) WHERE a.TS_CustomerID = ?");
            pst.setInt(1, khid);
            rsCus = pst.executeQuery();
            while (rsCus.next()) {
                tbTransaction record = new tbTransaction(rsCus.getInt(1), rsCus.getInt(9), rsCus.getInt(5), rsCus.getInt(4), rsCus.getLong(6), rsCus.getTimestamp(7), rsCus.getString(2), rsCus.getString(3));
    ...
    public ArrayList<tbTransaction> getTransactionBYCardID(int cardid) {
        ...
        try {
            cn = MyConnection.getConnect(server.getServerName(), server.getDatabaseName(), server.getUserName(), server.getPassword());
            pst = cn.prepareCall("SELECT a.ID, b.Cus_Name, c.ATM_Name, d.ATMCardID, a.TS_Manipulation, a.TS_Money, a.TS_Date, a.TS_Status, a.TS_Customer_Sub FROM (((GiaoDich a JOIN KhachHang b ON a.TS_CustomerID = b.ID) JOIN ATM c ON a.ATMID = c.ID) JOIN ATMCard d ON a.ATMCardID = d.ID) WHERE a.ATMCARDID = ?");
            pst.setInt(1, cardid);
            rsCus = pst.executeQuery();
            while (rsCus.next()) {
                tbTransaction record = new tbTransaction(rsCus.getInt(1), rsCus.getInt(9), rsCus.getInt(5), rsCus.getInt(4), rsCus.getLong(6), rsCus.getTimestamp(7), rsCus.getString(2), rsCus.getString(3));
