...
public class tbDeposit {
    ...
    public ArrayList<tbDeposit> getList() {
        ...
        try {
            ...
            pst = cn.prepareCall("SELECT a.ID, d.Cus_Name, a.TS_Money, b.ATM_Name, c.ATMCardID, a.TS_Date, d.id FROM ((GiaoDich a INNER JOIN ATM b ON a.ATMID = b.ID) INNER JOIN ATMCard c ON a.ATMCardID = c.ID) INNER JOIN KhachHang d ON a.TS_CustomerID = d.ID WHERE a.TS_Manipulation = 0 AND a.TS_Status = 0");
    ...
    public tbDeposit getInfo(int id) {
        ...
        try {
            cn = MyConnection.getConnect(server.getServerName(), server.getDatabaseName(), server.getUserName(), server.getPassword());
            pst = cn.prepareCall("SELECT a.ID, d.Cus_Name, a.TS_Money, b.ATM_Name, c.ATMCardID, a.TS_Date, d.id FROM ((GiaoDich a INNER JOIN ATM b ON a.ATMID = b.ID) INNER JOIN ATMCard c ON a.ATMCardID = c.ID) INNER JOIN KhachHang d ON a.TS_CustomerID = d.ID WHERE a.TS_Manipulation = 0 AND a.id = ? AND a.TS_Status = 0");
            pst.setInt(1, id);
            rsCus = pst.executeQuery();
            if (rsCus.next()) {
    ...
    public boolean accept(long money, int khid, int tranid) {
        ...
        try {
            cn = MyConnection.getConnect(server.getServerName(), server.getDatabaseName(), server.getUserName(), server.getPassword());
            pst = cn.prepareCall("UPDATE [dbo].[KhachHang] SET [Cus_Blance] = Cus_Blance + ? WHERE ID = ?");
            //SoDu
            pst.setLong(1, money);
            pst.setInt(2, khid);
            result = pst.executeUpdate();
            if (result > 0) {
                psd = cn.prepareCall("UPDATE [dbo].[GiaoDich] SET [TS_Status] = 1 WHERE ID = ?");
                psd.setInt(1, tranid);
